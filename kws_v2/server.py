...
from model import SoundClassificationV2
...
async def model_inference(websocket: WebSocket):
    ...
    infer_time = 50        # 预设推理时长ms，决定接受几次二进制包推理一次
    ...
    try: 
        while True:
            ...
            if "text" in data.keys():
                ...
                if "messageType" not in message.keys():
                    logger.warning(f"{Logger.file_lineno()} 收到无效数据: {data}")
                    await websocket.send_text(KWSResponse(messageType="inference", code=1).model_dump_json())
                    continue
                message_type = message.get("messageType")
                if message_type != "inference":
                    logger.warning(f"{Logger.file_lineno()} 错误的messageType: {data}")
                    await websocket.send_text(KWSResponse(messageType="inference", code=2).model_dump_json())
                    continue
                try: 
                    ...
                    logger.warning(f"{Logger.file_lineno()} 参数传递有误：{data}",exc_info=True)
                    await websocket.send_text(KWSResponse(messageType="inference", code=3).model_dump_json())
                    continue
                ...
                if buffer_count*time_per_buffer >= infer_time:
                    ...
                    if res == 0:
                        ...
                        # write(f"dump/{end_infer}.wav", 8000, input_data)
