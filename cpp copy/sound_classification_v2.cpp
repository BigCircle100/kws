...
SoundClassificationV2::SoundClassificationV2(int dev_id, 
                                            ...
                                            float threshold) : m_model_threshold(threshold) {
...
// 目前不确定模型的输入输出tensor个数，后续拿到bmodel再改
  ...
  h_input_size = input_shape_size*sizeof(uint8_t);
...
int SoundClassificationV2::inference(short *temp_buffer, int buffer_size,int *index) {

  normal_sound(temp_buffer, buffer_size);
  mp_extractor_->update_data(temp_buffer, buffer_size);

  mp_extractor_->melspectrogram_optimze(temp_buffer, buffer_size, static_cast<int8_t*>(m_input_buffers[0]),
  ...
  *index = get_top_k(static_cast<float*>(m_output_buffers[0]), m_output_element);
